...
public class RewardAppController {
     ...
     */
    @ResponseBody
    @RequestMapping("v1/appApi/queryRechargeRules")
    public ResponseObject queryRechargeRules(@PathVariable String companyId, @PathVariable String appCode, @PathVariable String userId, @PathVariable String serviceCode, @Validated CommonParameters commonParameters, @RequestBody String strJson) {
        ResponseObject resObj = new ResponseObject();
        logger.info(ValidateCommonParam.getRequestInfo(companyId, appCode, userId, serviceCode, strJson));
        try {
            //数据解析
            ...
            boolean validate = ValidateCommonParam.validateCommonParam(commonParameters, jsonMap, errorMap);
            if (!validate) {
                logger.error("class : RewardAppController --> method : queryRechargeRules " + errorMap.get(Constants.MESSAGE));
                ValidateCommonParam.parameterError(resObj, errorMap.get(Constants.MESSAGE));
                return resObj;
            }
            //用户非空校验
            boolean checkFans = rewardAppService.checkFans(commonParameters);
            if (!checkFans) {
                logger.error("class : RewardAppController --> method : queryRechargeRules " + GeneralStatus.user_error.enDetail);
                ValidateCommonParam.userError(resObj);
                return resObj;
            }
            
            resObj = rewardAppService.queryRechargeRules(commonParameters, jsonMap);
        } catch (Exception e) {
        	e.printStackTrace();
            ValidateCommonParam.innerError(resObj);
            logger.error("class : RewardAppController --> method : queryRechargeRules处理异常！");
        }
        return resObj;
    }
}
